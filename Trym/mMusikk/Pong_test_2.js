function pong(spillere) {
  ...
  Ball.prototype.update = function(paddle1, paddle2){
    ...
    if (spillere == 1) {
      ...
      if (poeng_spiller_1 === 7) {
        ...
        vinnSpill();
        if (!pausetmusikk) {
        var taperMusikk = new Audio('loser.wav');
        taperMusikk.play();
      }
      }
    }
    ...
    if (topp_x > bredde/2) {
      if (topp_x < (paddle1.x + paddle1.width) && bunn_x > paddle1.x && topp_y < (paddle1.y + paddle1.height) && bunn_y > paddle1.y) {
        ...
        this.x += this.x_fart;
        if (!pausetSFX) {
        var sprett1 = new Audio('bounce1.wav');
        sprett1.play();
       }
      }
    } else {
      if (topp_x < (paddle2.x + paddle2.width) && bunn_x > paddle2.x && topp_y < (paddle2.y + paddle2.height) && bunn_y > paddle2.y) {
        ...
        this.x += this.x_fart;
        if (!pausetSFX) {
        var sprett2 = new Audio('bounce2.wav');
        sprett2.play();
       }
      }
    }
  ...
  function startMusic(){
  myMusic = new sound("BeepBox-Song.wav", "true", 0.5);
